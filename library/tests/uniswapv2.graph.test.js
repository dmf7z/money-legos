...
describe("UniswapV2 Graph", function() {
  ...
  before(async function() {
    contracts = await Deployer.deploy(true, false, true);
  ...
  it("UniswapV2 USDC WETH Graph", async function() {
    ...
    let id1 = graph.addElement(element, 0, 0, [
      {
        index: 1,
        value: "100000", //usdcBalance.toString(10),
      },
    ]);
  ...
  it("UniswapV2 WETH DAI Graph", async function() {
    const daiContract = new web3.eth.Contract(ERC20ABI, contracts.ASSETS.DAI);
    ...
    let id1 = graph.connectElements([[id0, 0, 0]], element, 0, 1);

    //Create operation and connect them
    element = elements.OP_UNISWAP_V2_WETH_TO_DAI;
    let id2 = graph.connectElements([[id1, 0, 0]], element, 0, 1);
    ...
    await loadedGraph.allowInputElement(web3, id0);
