...
describe("Operation Curve", function() {
  let contracts;
  ...
  it("Trade DAI for SUSD", async function() {
    ...
    //Trade in Curve  DAI for USD
    params = ABICoder.encodeParameters(
      ["address", "address", "int128", "int128", "uint256"],
      [
    ...
    await executorContract.methods
      .executeOperation(contracts.OPERATIONS.OP_CURVE, [daiBalance], params)
      .send({
        from: accounts[0],
        gas: 1500000,
      });
